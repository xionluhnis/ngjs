        ...
        if (value === undefined) {
          ...
          if (isString(value)) {
            ...
            if (lastCookies.length > 20) {
              $log.warn("Cookie '" + name + "' possibly not set or overflowed because too many cookies " + "were already set (" + lastCookies.length + " > 20 )");
            }
          }
        }
      } else {
        ...
        for (var i = 0, ii = directives.length; i < ii; i++) {
          ...
          if (directiveValue = directive.transclude) {
            ...
            if (directiveValue == 'element') {
              ...
              $compileNode = templateAttrs.$$element = jqLite('<!-- ' + directiveName + ': ' + templateAttrs[directiveName] + ' -->');
          ...
          if (newScopeDirective && isObject(newScopeDirective.scope)) {
            ...
            forEach(newScopeDirective.scope, function(definiton, scopeName) {
              ...
              switch (mode) {
                ...
                case '=':
                  {
                    ...
                    parentSet = parentGet.assign || function() {
                      ...
                      throw Error(NON_ASSIGNABLE_MODEL_EXPRESSION + attrs[attrName] + ' (directive: ' + newScopeDirective.name + ')');
                    };
                    lastValue = scope[scopeName] = parentGet(parentScope);
                ...
                default:
                  {
                    throw Error('Invalid isolate scope definition for directive ' + newScopeDirective.name + ': ' + definiton);
                  }
              }
            });
          }
 ...
 */
  ...
  forEach(['src', 'href', 'title'], function(attrName) {
    var normalized = directiveNormalize('ng-' + attrName);
    ngAttributeAliasDirectives[normalized] = function() {
      return {
      ...
      scope.$watch(attr[name], function(newVal, oldVal) {
        if (selector === true || scope.$index % 2 === selector) {
          if (oldVal && (newVal !== oldVal)) {
            if (isObject(oldVal) && !isArray(oldVal)) oldVal = map(oldVal, function(v, k) {
              if (v) return k
            });
            element.removeClass(isArray(oldVal) ? oldVal.join(' ') : oldVal);
          }
          if (isObject(newVal) && !isArray(newVal)) newVal = map(newVal, function(v, k) {
            if (v) return k
          });
 ...
 */
  ...
  forEach('click dblclick mousedown mouseup mouseover mouseout mousemove mouseenter mouseleave'.split(' '),
 ...
 */
   ...
   */
  var ngViewDirective = ['$http', '$templateCache', '$route', '$anchorScroll', '$compile', '$controller',
